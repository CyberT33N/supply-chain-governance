...
func TestRunSuccessExecutesEveryGate(t *testing.T) {
	...
	for _, required := range []string{
		...
		"go test -mod=readonly ./internal/capabilitypack -run=^$ -fuzz=FuzzParsePack -fuzztime=50000x -parallel=1",
		"go test -mod=readonly ./internal/qualitygateconfig -run=^$ -fuzz=FuzzParseConfig -fuzztime=50000x -parallel=1",
