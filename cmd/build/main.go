// Command build runs the Supply Chain Governance source-level quality gates.
package main
...
const linuxSourceGatePath = ".build/bin/supply-chain-governance-source-gate"
...
func run(
	...
	fmt.Fprintln(stdout, "Supply Chain Governance source-level build completed successfully.")
	return 0
}
...
func sourceQualitySteps() []step {
	return []step{
		...
		},
		{
			...
			executable: "go",
			arguments:  []string{"run", "-mod=readonly", "./cmd/check-conformance"},
		},
		{
		...
		{
			name:       "fuzz capability pack parser",
			executable: "go",
			arguments:  []string{"test", "-mod=readonly", "./internal/capabilitypack", "-run=^$", "-fuzz=FuzzParsePack", "-fuzztime=50000x", "-parallel=1"},
		},
		{
			name:       "fuzz quality gate config parser",
			executable: "go",
			arguments:  []string{"test", "-mod=readonly", "./internal/qualitygateconfig", "-run=^$", "-fuzz=FuzzParseConfig", "-fuzztime=50000x", "-parallel=1"},
		},
		{
...
func linuxBuildSteps() []step {
	return []step{
		{
			name:        "build Linux AMD64 Supply Chain Governance source gate",
		...
		{
			name:       "record Linux Supply Chain Governance module provenance",
			executable: "go",
