...
These files map the canonical Git governance to the current Supply Chain
Governance source boundary. They do not define a second release, evidence, or
dependency policy.
...
The Linux quality gate enforces formatting, module integrity, tests,
conformance vectors, exact 100% statement coverage, race detection, static
analysis, Linux/AMD64 build, and module provenance.

## Security boundary

Ruleset files must not contain credentials, signing keys, bypass actors,
mutable references, or bindings to other organizations or tenants. The push
Ruleset carries the `source` field as the repository's own identity because
the GitHub import format requires it.
