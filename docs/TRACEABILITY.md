...
| SCG-10 | Converge the canonical pins onto the current home line: the three byte-identical callers, the binding manifest (home `sha` `38c8ea2`, the retired `version` field removed, the new caller hashes), and the conformance action reference move from the pre-exact-provisioning pin `f9c7eba` to the canonical pin `38c8ea2` whose payload resolves the tenant `toolchain` directive fail-closed (never the `go` directive, never the latest patch), and the tool pins rise to the merged go-quality-authority (`7f40c7b`) and repository-governance (`68550a9`) lines — closing the drift the Go 1.26.7 patch release activated. | In progress |
