...
| SCG-9 | Bind the schema-owned `includeFamilies` default into the centralized `quality-gate-config/v4` seam: the schema carries the canonical ticket-family set (`feature`, `fix`, `docs`, `refactor`, `chore`, `test`, `perf`, `hotfix`) as the default annotation of `defaults.includeFamilies`, so a tenant configuration omits the block unless it declares a named deviation; adds the strict reference decoder `internal/qualitygateconfig` (the default fills only an absent block or list, never a declared one), the seam conformance vectors proving the omitted and the deviating forms plus every rejection, the harness wiring in `check-conformance`, and the fourth parser fuzz lane. | In progress |
