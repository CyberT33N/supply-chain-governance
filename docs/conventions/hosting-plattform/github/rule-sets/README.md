...
Dieses Projekt (`supply-chain-governance`) verwendet die Familie
**`quality-gates=linux-only`**:

- Die Quality Gates laufen ausschließlich für **Linux**.
- Architektonische Begründung: Dieses Projekt liefert portable
  JSON-Schema-Artefakte, Konformitätsvektoren und versionierte Policies sowie
  reine Linux-/CI-/CD-Artefakte; das Gate-Tooling wird ausschließlich für
  Linux/AMD64 gebaut und verifiziert. Es existieren keine
